# Hugging Face Deployment Plan — GROWW Weekly Review Pulse Agent

## Overview

This document provides a complete deployment plan for deploying the GROWW Weekly Review Pulse Agent MCP server on Hugging Face Spaces. The deployment includes Google Drive/Docs and Gmail MCP servers with OAuth 2.0 authentication, and supports both OpenAI and GROQ LLM providers.
...
3. **GitHub Repository**: Your project code pushed to GitHub (Hugging Face integrates with GitHub)
...
5. Create OAuth 2.0 credentials:
   ...
   - Add authorized redirect URIs (for Hugging Face, you'll add the Space URL later)
...
The following files are included for Hugging Face deployment:

### 1. `README.md`
Configuration file for Hugging Face Spaces with YAML frontmatter for deployment settings.

### 2. `Dockerfile`
Docker container configuration for running the MCP server on Hugging Face Spaces.
...
### Step 2: Create New Space on Hugging Face

1. Log in to [huggingface.co](https://huggingface.co)
2. Click "Create new Space"
3. Select "Docker" as the Space type
4. Connect your GitHub repository or paste the repository URL
5. Hugging Face will automatically detect the `README.md` configuration

### Step 3: Configure Environment Variables

1. In Hugging Face, go to your Space settings
2. Navigate to the "Settings" tab and then "Variables"
...
4. Add your Hugging Face Space URL as an authorized redirect URI:
   - Format: `https://your-space-name.huggingface.co/`
5. Save the changes
6. Download the updated `credentials.json`
7. Update the `GOOGLE_CREDENTIALS_JSON` environment variable in Hugging Face

### Step 5: Deploy

1. In Hugging Face, click "Create Space" or "Deploy"
2. Hugging Face will build the Docker image and deploy your application
3. Monitor the deployment logs for any errors
4. Once deployed, you'll receive a Hugging Face URL for your MCP server

### Step 6: Verify Deployment

1. Check the Hugging Face logs to ensure the MCP server started successfully
...
After deployment, update your local `mcp-config.json` to use the Hugging Face URLs:

```json
{
  "gdrive_mcp": {
    "server": "https://your-space-name.huggingface.co",
    "transport": "http"
  },
  "gmail_mcp": {
    "server": "https://your-space-name.huggingface.co",
...
1. Enable Hugging Face's built-in metrics and logging
2. Set up alerts for:
   - High error rates
...
1. Use Hugging Face cron jobs or external scheduler (e.g., GitHub Actions)
2. Configure the scheduler to trigger the agent pipeline
3. Ensure the agent can access the Hugging Face-deployed MCP servers
...
2. Update the corresponding environment variable in Hugging Face
3. Redeploy
...
### Hugging Face Build Failures

If the build fails:

1. Check the Hugging Face build logs
2. Ensure all dependencies in `requirements.txt` are correct
3. Verify the Dockerfile is compatible with Hugging Face's build environment
...
2. Check Hugging Face logs for authentication errors
3. Ensure Google OAuth credentials have the correct scopes
4. Verify the Hugging Face URL is added to Google OAuth redirect URIs
...
2. **Use Hugging Face's secret management**: Store sensitive data in Hugging Face environment variables, not in code
3. **Rotate tokens regularly**: OAuth tokens should be refreshed periodically
4. **Monitor access**: Keep track of who has access to your Hugging Face Space and Google Cloud Console
5. **Use HTTPS**: Ensure all MCP server communications use HTTPS

## Cost Estimation

Hugging Face Spaces pricing:
- Free tier: Available with basic CPU (limited resources)
- Paid tiers: Starting at $9/month for better hardware
...
1. **Monitor logs**: Check Hugging Face logs for errors or warnings
...
3. Hugging Face will automatically redeploy on push (or trigger manual deploy)
...
5. Document the Hugging Face URL for your team
